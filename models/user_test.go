...
func testUserToOneDepartmentUsingDepartment(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadDepartment(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToOneSetOpDepartmentUsingDepartment(t *testing.T) {
	var err error
...
func testUserToOneRemoveOpDepartmentUsingDepartment(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `DepartmentID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
